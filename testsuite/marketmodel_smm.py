...
class CommonVars(object):
    ...
    def setup(self):
        ...
        self.c_ = 0.5403
        self.d_ = 0.1710
        self.volatilities_ = DoubleVector(len(self.todaysSwaps_))
        self.blackVols_ = DoubleVector(len(self.todaysSwaps_))
        for i in range(len(self.todaysSwaps_)):
            self.volatilities_[i] = self.todaysSwaps_[i] * mktVols[i] / (self.todaysSwaps_[i] + self.displacement_)
    ...
    def makeMarketModel(self,
        ...
        bumpedRates = DoubleVector(len(self.todaysForwards_))
        curveState_lmm = LMMCurveState(self.rateTimes_)
        curveState_lmm.setOnForwardRates(self.todaysForwards_)
        usedRates = curveState_lmm.coterminalSwapRates()

        for i in range(len(usedRates)):
            bumpedRates[i] += usedRates[i]

        bumpedVols = DoubleVector(self.volatilities_.size())

        for i in range(len(self.volatilities_)):
            bumpedVols[i] += self.volatilities_[i]
        correlations = exponentialCorrelations(
            evolution.rateTimes(),
            self.longTermCorrelation_,
            self.beta_)
        corr = TimeHomogeneousForwardCorrelation(
            correlations, evolution.rateTimes())
...
class MarketModelSmmTest(unittest.TestCase):

    def testMultiStepCoterminalSwapsAndSwaptions(self):
        ...
        displacedPayoff = StrikedTypePayoffVector(len(vars.todaysSwaps_))
        undisplacedPayoff = StrikedTypePayoffVector(len(vars.todaysSwaps_))
                ...
                for measure in measures:
                    ...
                    for i in range(len(evolvers) - stop):
                        for n in range(1):
                            ...
                            self.checkCoterminalSwapsAndSwaptions(
                                stats, fixedRate,
                                displacedPayoff, marketModel, vars)

    def makeMeasure(self, product,
                    measureType, vars):
        result = None
        evolution = product.evolution()
        ...
        if measureType == MeasureType.MoneyMarketPlus:
            result = moneyMarketPlusMeasure(
                evolution, vars.measureOffset_)
            self.assertFalse(not isInMoneyMarketPlusMeasure(evolution, result, vars.measureOffset_))

        checkCompatibility(evolution, result)

        return result

    def checkCoterminalSwapsAndSwaptions(
            self,
            stats,
            fixedRate,
            displacedPayoff,
        ...
        expectedNPVs = DoubleVector(len(vars.todaysSwaps_))
        errorThreshold = 0.5
        for i in range(N):
            expectedNPV = curveState_lmm.coterminalSwapAnnuity(i, i) * (vars.todaysSwaps_[i] - fixedRate) * vars.todaysDiscounts_[i]
        ...
        for i in range(N):
            expectedSwaption = BlackCalculator(
                displacedPayoff[i],
                vars.todaysSwaps_[i] + vars.displacement_,
