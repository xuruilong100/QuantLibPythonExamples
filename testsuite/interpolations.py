...
class InterpolationTest(unittest.TestCase):
    ...
    def testKernelInterpolation(self):
        ...
        yd2 = DoubleVector(len(deltaGrid))
        yd3 = DoubleVector(len(deltaGrid))
    ...
    @unittest.skip("testKernelInterpolation2D: crash randomly")
    ...
    def testUnknownRichardsonExtrapolation(self):
        TEST_MESSAGE(
            "Testing Richardson extrapolation with "
    ...
    def testNoArbSabrInterpolation(self):
        ...
        betaGuess = 0.5
        alphaGuess = 0.2 / pow(forward, betaGuess - 1.0)
        nuGuess = sqrt(0.4)
        rhoGuess = 0.0
    ...
    def testFlochKennedySabrIsSmoothAroundATM(self):
        TEST_MESSAGE(
            "Testing that Andersen SABR formula is smooth "
    ...
    def testLeFlochKennedySabrExample(self):
        TEST_MESSAGE(
            "Testing Le Floc'h Kennedy SABR Example...")
    ...
    def testLagrangeInterpolationDerivative(self):
        ...
        eps = sqrt(QL_EPSILON)
        for i in np.arange(-1.0, 0.9 + 0.01, 0.01):
    ...
    @unittest.skip("testBackwardFlatOnSinglePoint: fails on ubuntu")
    ...
    def _checkSymmetry(self,
                       ...
                       xMin):
        tolerance = 1.0e-15
        for x in np.arange(xMin, 0.0, 0.1):
            y1 = cubic(x)
            y2 = cubic(-x)
