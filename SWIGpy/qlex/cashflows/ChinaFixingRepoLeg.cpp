...
namespace QuantLib {
    ...
    : schedule_(schedule), chinaFixingRepo_(std::move(chinaFixingRepo)), notional_(Null<Real>()),
      paymentCalendar_(schedule.calendar()), paymentConvention_(ModifiedFollowing), paymentLag_(0),
      gearing_(1.0), spread_(0.0) {}
    ...
    ChinaFixingRepoLeg&
    ChinaFixingRepoLeg::withPaymentAdjustment(BusinessDayConvention convention) {
    ...
    ChinaFixingRepoLeg::operator Leg() const {
        ...
        for (Size i = 0; i < n; ++i) {
            ...
            if (i == 0 && schedule_.hasIsRegular() && !schedule_.isRegular(i + 1)) {
                refStart = calendar.adjust(end - schedule_.tenor(), paymentConvention_);
            }
            if (i == n - 1 && schedule_.hasIsRegular() && !schedule_.isRegular(i + 1)) {
                refEnd = calendar.adjust(start + schedule_.tenor(), paymentConvention_);
            }

            cashflows.push_back(ext::shared_ptr<CashFlow>(new ChinaFixingRepoCoupon(
                paymentDate, notional_, start, end, chinaFixingRepo_, gearing_, spread_, refStart,
                refEnd, paymentDayCounter_)));
        }
        return cashflows;
    }
