...
namespace QuantLib {
    ...
    class Actual365_25 : public DayCounter {
      public:
        ...
        Actual365_25(Convention c = Actual365_25::Standard) : DayCounter(implementation(c)) {}

      private:
        class Impl : public DayCounter::Impl {
          public:
            std::string name() const { return std::string("Actual/365.25 (Fixed)"); }
            Time yearFraction(const Date& d1, const Date& d2, const Date&, const Date&) const {
                return daysBetween(d1, d2) / 365.25;
            }
        };
        class CA_Impl : public DayCounter::Impl {
          public:
            std::string name() const { return std::string("Actual/365.25 (Fixed) Canadian Bond"); }
        ...
        class NL_Impl : public DayCounter::Impl {
          public:
            std::string name() const { return std::string("Actual/365.25 (No Leap)"); }
