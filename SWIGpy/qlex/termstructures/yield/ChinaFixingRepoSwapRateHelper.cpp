...
namespace QuantLib {
    ChinaFixingRepoSwapRateHelper::ChinaFixingRepoSwapRateHelper(
    ...
    : RelativeDateRateHelper(fixedRate), pillarChoice_(pillar), settlementDays_(settlementDays),
      tenor_(tenor), chinaFixingRepo_(std::move(chinaFixingRepo)),
      discountHandle_(std::move(discount)), paymentLag_(paymentLag),
      paymentConvention_(paymentConvention), paymentFrequency_(paymentFrequency),
      paymentCalendar_(std::move(paymentCalendar)), forwardStart_(forwardStart), gearing_(gearing),
      spread_(spread) {
    ...
    void ChinaFixingRepoSwapRateHelper::initializeDates() {
        ...
        // review here
        ext::shared_ptr<IborIndex> clonedIborIndex = chinaFixingRepo_->clone(termStructureHandle_);
        ...
        swap_ = MakeChinaFixingRepoSwap(tenor_, clonedChinaFixingRepo, 0.0, forwardStart_)
        ...
        Date lastPaymentDate =
            std::max(swap_->floatingLeg().back()->date(), swap_->fixedLeg().back()->date());
        ...
        switch (pillarChoice_) {
            ...
            case Pillar::CustomDate:
                ...
                QL_REQUIRE(
                    ...
                    "pillar date ("
                        << pillarDate_
                        << ") must be later than or equal to the instrument's earliest date ("
                        << earliestDate_ << ")");
                QL_REQUIRE(
                    pillarDate_ <= latestRelevantDate_,
                    "pillar date ("
