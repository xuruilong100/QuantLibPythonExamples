...
namespace QuantLib {
    ...
    BlissFitting::BlissFitting(const Array& weights, const Array& l2)
    ...
    DiscountFactor BlissFitting::discountFunction(const Array& x, Time t) const {
        Real kappa = x[size() - 2];
        Real kappa_1 = x[size() - 1];

        Real zeroRate =
            x[0] + x[1] * (1.0 - std::exp(-kappa * t)) / ((kappa + QL_EPSILON) * (t + QL_EPSILON)) +
