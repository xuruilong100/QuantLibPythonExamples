...
namespace QuantLib {
    ...
    DieboldLiFitting::DieboldLiFitting(Real kappa,
                                       ...
                                       const Array& l2)
    : FittedBondDiscountCurve::FittingMethod(true, weights, optimizationMethod, l2), kappa_(kappa) {
    }

    DieboldLiFitting::DieboldLiFitting(Real kappa, const Array& weights, const Array& l2)
    ...
    DiscountFactor DieboldLiFitting::discountFunction(const Array& x, Time t) const {
        Real zeroRate = x[0] +
                        (x[1] + x[2]) * (1.0 - std::exp(-kappa_ * t)) /
                            ((kappa_ + QL_EPSILON) * (t + QL_EPSILON)) -
