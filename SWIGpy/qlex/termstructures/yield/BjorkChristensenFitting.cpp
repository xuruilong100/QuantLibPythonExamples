...
namespace QuantLib {
    ...
    DiscountFactor BjorkChristensenFitting::discountFunction(const Array& x, Time t) const {
        Real kappa = x[size() - 1];

        Real zeroRate = x[0] +
                        (x[1] + x[2]) * (1.0 - std::exp(-kappa * t)) /
                            ((kappa + QL_EPSILON) * (t + QL_EPSILON)) -
                        x[2] * std::exp(-kappa * t) +
                        x[3] * (1.0 - std::exp(-kappa * t * 2.0)) /
                            ((kappa + QL_EPSILON) * (t + QL_EPSILON) * 2.0);
