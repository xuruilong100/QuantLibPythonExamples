...
namespace QuantLib {
    ...
    AdjustedSvenssonFitting::AdjustedSvenssonFitting(
        const Array& weights,
        ext::shared_ptr<OptimizationMethod> optimizationMethod,
        const Array& l2)
    : FittedBondDiscountCurve::FittingMethod(true, weights, optimizationMethod, l2) {}

    AdjustedSvenssonFitting::AdjustedSvenssonFitting(const Array& weights, const Array& l2)
    ...
    DiscountFactor AdjustedSvenssonFitting::discountFunction(const Array& x, Time t) const {
        Real kappa = x[size() - 2];
        ...
        Real zeroRate =
            ...
            (x[2]) * std::exp(-kappa * t) +
            x[3] * (((1.0 - std::exp(-kappa_1 * t)) / ((kappa_1 + QL_EPSILON) * (t + QL_EPSILON))) -
                    std::exp(-kappa_1 * t * 2.0));
