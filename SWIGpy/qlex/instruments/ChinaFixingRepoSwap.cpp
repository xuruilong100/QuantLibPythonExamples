...
namespace QuantLib {
    ...
    : Swap(2), type_(type), nominal_(nominal), paymentFrequency_(schedule.tenor().frequency()),
      paymentCalendar_(paymentCalendar.empty() ? schedule.calendar() : paymentCalendar),
      paymentConvention_(paymentAdjustment), paymentLag_(paymentLag), schedule_(schedule),
      fixedRate_(fixedRate), fixedDC_(std::move(fixedDC)),
      chinaFixingRepo_(std::move(chinaFixingRepo)), gearing_(gearing), spread_(spread) {
