...
namespace QuantLib {
    ...
    MakeChinaFixingRepoSwap::MakeChinaFixingRepoSwap(
        const Period& swapTenor,
        const ext::shared_ptr<ChinaFixingRepo>& chinaFixingRepo,
        Rate fixedRate,
        const Period& forwardStart)
    : swapTenor_(swapTenor), chinaFixingRepo_(chinaFixingRepo), fixedRate_(fixedRate),
      forwardStart_(forwardStart), settlementDays_(1), calendar_(chinaFixingRepo->fixingCalendar()),
      paymentFrequency_(Quarterly), paymentCalendar_(Calendar()),
      paymentConvention_(ModifiedFollowing), paymentLag_(0), rule_(DateGeneration::Forward),
      isDefaultEOM_(false), type_(ChinaFixingRepoSwap::Payer), nominal_(100.0), gearing_(1.0),
      spread_(0.0), fixedDayCount_(chinaFixingRepo->dayCounter()) {}
    ...
    MakeChinaFixingRepoSwap::operator ext::shared_ptr<ChinaFixingRepoSwap>() const {
        ...
        if (effectiveDate_ != Date())
            ...
            Date spotDate = calendar_.advance(refDate, settlementDays_ * Days);
        ...
        if (endDate == Date()) {
            if (usedEndOfMonth) {
                endDate =
                    calendar_.advance(startDate, swapTenor_, ModifiedFollowing, usedEndOfMonth);
            } else {
                endDate = startDate + swapTenor_;
            }
        }

        Schedule schedule(startDate, endDate, Period(paymentFrequency_), calendar_,
                          ModifiedFollowing, ModifiedFollowing, rule_, usedEndOfMonth);

        Rate usedFixedRate = fixedRate_;
        if (fixedRate_ == Null<Rate>()) {
            ChinaFixingRepoSwap temp(type_, nominal_, schedule,
                                     0.0, // fixed rate
                                     fixedDayCount_, chinaFixingRepo_, gearing_, spread_,
                                     paymentLag_, paymentConvention_, paymentCalendar_);
            if (engine_ == nullptr) {
                Handle<YieldTermStructure> disc = chinaFixingRepo_->forwardingTermStructure();
                QL_REQUIRE(!disc.empty(), "null term structure set to this instance of "
                                              << chinaFixingRepo_->name());
                bool includeSettlementDateFlows = false;
                ext::shared_ptr<PricingEngine> engine(
                    new DiscountingSwapEngine(disc, includeSettlementDateFlows));
        ...
        ext::shared_ptr<ChinaFixingRepoSwap> swap(new ChinaFixingRepoSwap(
            type_, nominal_, schedule, usedFixedRate, fixedDayCount_, chinaFixingRepo_, gearing_,
            spread_, paymentLag_, paymentConvention_, paymentCalendar_));
    ...
    MakeChinaFixingRepoSwap::withTerminationDate(const Date& terminationDate) {
    ...
    MakeChinaFixingRepoSwap&
    MakeChinaFixingRepoSwap::withDiscountingTermStructure(const Handle<YieldTermStructure>& d) {
        bool includeSettlementDateFlows = false;
    ...
    MakeChinaFixingRepoSwap&
    MakeChinaFixingRepoSwap::withPricingEngine(const ext::shared_ptr<PricingEngine>& engine) {
        engine_ = engine;
        return *this;
    }
